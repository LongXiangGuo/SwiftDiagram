...
extension SyntaxStructure {
    ...
    /// 递归向上查找已命中的规则（嵌套类型继承顶层类型的规则）
    var effectiveResolvedRule: ResolvedRule? {
        resolvedRule ?? parent?.effectiveResolvedRule
    }

    /// 递归向上查找文件路径（嵌套类型与顶层类型同文件）
    var effectiveFilePath: String? {
        filePath ?? parent?.effectiveFilePath
    }

    /// 类型所属分组名（未分组为 `nil`）
    var groupName: String? {
        effectiveResolvedRule?.groupName
    }
}
